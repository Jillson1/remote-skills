...
**按日期区间导出**：传入 `--date` 时，只导出关注周数据；同时传入 `--compare-date` 时会再导出对比周数据，两个 CSV 会复制到 `cache` 供后续分析使用。**对比周每次都会重新导出**（不使用已有文件缓存），以避免与关注周误用同一份数据。此时脚本会**自动**生成以下 UTF-8 文件（均不依赖控制台编码，避免中文乱码）：
- `cache/feedback_compare_stats.txt`：按二级分类的统计与环比增长率，Step 2 可直接读取填写「结论总结」与「结论对比表」；
- `cache/export_summary_<区间>.txt`：当次导出的行数、周报日期/月份抽样，便于核对导出区间；
- `cache/export_verify.txt`：关注周与对比周行数及「周报日期」抽样；若两期完全一致会写入 WARNING，需检查源表「周报日期」列或导出参数。
...
**Script Actions**:
1.  **Export then Read (优先)**：使用本 Skill 内置的 WPS 客户端将 Sheet1 按日期区间导出到 `pdf-feedback-report/cache` 下的 CSV，并从该文件解析数据。脚本会**自动识别真实表头**：若源表第一行为空、第二行为表头，会以第二行作为表头，确保「周报日期」等列正确，再按指定周报日期过滤，避免误导出全量数据。
...
### Step 3: 发布为在线文档 (Publish)（必须自动执行，不得询问用户）
报告生成完成后，**必须立即自动**使用 `AirpageFile` Skill 将报告发布为 WPS 在线智能文档并获取分享链接。**禁止**在此时询问用户是否发布；执行 Step 2 后直接执行本步骤。
...
**若出现「未进行实名认证」导致分享失败**：文档仍会创建成功并写入内容，但无法自动开启分享链接。**实名认证需在 [WPS 开放平台](https://open.wps.cn) 控制台完成**（企业/个人实名等），无法通过脚本自动认证；认证完成后重新发布或在该文档上手动开启分享即可。

**若 AirpageFile Skill 不存在**（如目录 `.claude/skills/AirpageFile/` 或 `AirpageFile/scripts/publish_doc.py` 不存在）：请提示用户前往 **http://10.13.153.85/skills** 下载并安装 AirpageFile Skill 后，再执行发布。

### Step 4: 清理 cache（报告生成后自动执行）
Step 3 完成后，**必须自动**执行 cache 清理，使 `pdf-feedback-report/cache` 中仅保留 `.csv` 文件，删除所有 `.txt` 与 `.md` 文件（如 `feedback_context.md`、`feedback_compare_stats.txt`、`export_summary_*.txt`、`export_verify.txt` 等），无需询问用户。

**Command**:
```bash
python .claude/skills/pdf-feedback-report/scripts/clean_cache.py
```
（工作区根路径为 `e:\codee\aiskills` 时，可先 `Set-Location e:\codee\aiskills` 再执行；或使用相对于工作区根的路径。）

**Result**: cache 目录下仅保留 `.csv` 数据文件，便于下次执行时减少冗余文件。

## 🛠 File Structure

-   `config/config.properties`: 配置文件（数据源 URL 等）。
-   `scripts/process_feedback.py`: 核心处理脚本。
-   `scripts/clean_cache.py`: 清理脚本（报告生成后删除 cache 内 .txt/.md，仅保留 .csv）。
